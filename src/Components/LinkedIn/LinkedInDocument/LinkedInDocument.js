...
const [disableDocument, setDisableDocument] = useState(false);
  ...
  const savePageDataAndShare = () => {
    ...
    summaryId
      ? handleShowShareDialog('https://cvjury.app/linkedIn_view/edit/' +summaryId +'/share') // share_Modal('https://cvjury.app/linkedIn_view/edit/' +summaryId +'/share')
      : 
      create_user_linkedIn_Summary(saveLinkedInData, config).then((res) => {
          if (res?.status === 200 || res?.status === 304) {
            setShowToastText('Saved Successfully!');
            setLoading(false);
            setShowSaveToast(true);
            handleShowShareDialog('https://cvjury.app/linkedIn_view/edit/' +res?.data.id +'/share')
            // share_Modal('https://cvjury.app/linkedIn_view/edit/' +res?.data.id +'/share');
          } else {
            // console.log(res);
            if (res?.includes(saveLinkedInData.documentName)) {
              setShowToastText(
  ...
  const aws_config = {
    ...
    dirName: 'linkedInSummaries' /* optional */,
    region: 'eu-central-1',
