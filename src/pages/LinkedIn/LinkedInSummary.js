...
import LinkedInDocument from '../../Components/LinkedIn/LinkedInDocument/LinkedInDocument';
...
export default function LinkedIn({
  ...
  const change_heading_Modal = (updatableId, headingId) => {
    confirmAlert({
      customUI: ({ onClose }) => {
        return (
          <div className='addPage_Modal login_modal'>
    ...
    <div className='builder'>
      ...
      <div style={{ overflowX: 'hidden', display: 'none' }}>
        ...
        >
          ...
          >
            <div
              style={{ marginTop: '0', fontSize: '12px', width: '180px' }}
            ...
            <div className='linkedin_chooseTemplate_main'>
              {linkedInHeadings ? (
                linkedInHeadings
                  ...
                  .map((heading, index) => (
                    <div className={`linkedin_template  `} key={index}>
                      <div
                        style={{ cursor: 'pointer', userSelect: 'none' }}
                        onClick={() => {
                          // change_heading_Modal(updatableId, heading.id);
                        }}
                      >
                        {removeMarkdown(heading.heading_data).substring(0, 50)}
                      </div>
                    </div>
                  ))
              ) : (
                  ...
                  >
                    {id ? (
                      ...
                      <div
                        ...
                        onClick={() => {
                          setLinkedInData('');
                          history.push(`/linkedIn/${template.id}`);
                          setUpdatableId(template.id);
                        }}
                      >
                        {template.template_name}
                      </div>
